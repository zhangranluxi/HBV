...
if __name__ == "__main__":
	...
	otu = pd.read_table(otutab, index_col=0, skiprows=0, header=1, sep="\t", comment=None)
	otu.pop("taxonomy")
	otu = otu.T
	sample = pd.read_table(samplefile, index_col=0, header=0, sep="\t", comment=None)
	tmptable = pd.concat([sample, otu], axis=1, join="inner")
